...
# ---- export (metadata-safe ZIP vs. direct attachment) ----------------------


def _prepared_session(interaction):
    """Run /metadata edit so a session + workspace exist, ready to export."""
    group = _group()
    att = FakeAttachment(payload=_png_bytes())
    _run(group.edit.callback(group, interaction, att))
    return dc.sessions.get(interaction.user.id)


def test_export_defaults_to_metadata_safe_zip():
    import zipfile

    interaction = make_interaction()
    session = _prepared_session(interaction)
    interaction.followup.send.reset_mock()

    _run(dc._do_export(interaction, session))
    ...
    with zipfile.ZipFile(zip_file.fp) as zf:
        names = set(zf.namelist())
        assert "metadata-report.json" in names
        assert "SHA256SUMS.txt" in names
        assert "README.txt" in names
        image_names = names - {"metadata-report.json", "SHA256SUMS.txt", "README.txt"}
        assert len(image_names) == 1


def test_export_direct_attachment_includes_warning():
    interaction = make_interaction()
    session = _prepared_session(interaction)
    session.config["direct_attachment"] = True
    interaction.followup.send.reset_mock()

    _run(dc._do_export(interaction, session))

    kwargs = interaction.followup.send.call_args.kwargs
    assert dc.DIRECT_ATTACHMENT_WARNING in kwargs["content"]
    assert not kwargs["files"][0].filename.endswith(".zip")
...
def test_edit_panel_has_picker_and_address_buttons():
    ...
    assert "Camera & resolution" in labels
