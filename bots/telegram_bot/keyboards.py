...
def exposure_keyboard() -> InlineKeyboardMarkup:
    """Camera-exposure choices shown after lens selection.

    Profile options are transparently *simulated* metadata; the review screen
    labels them as such.
    """
    from core.exposure.loader import get_default_profile_loader

    rows: list[list[InlineKeyboardButton]] = [
        [InlineKeyboardButton("Preserve existing", callback_data="exp:preserve")]
    ]
    for profile in get_default_profile_loader().list_profiles():
        rows.append([InlineKeyboardButton(profile.display_name, callback_data=f"exp:{profile.id}")])
    rows.append([InlineKeyboardButton("Custom values", callback_data="exp:custom")])
    return InlineKeyboardMarkup(rows)


def exposure_confirm_keyboard(profile_id: str) -> InlineKeyboardMarkup:
    """Confirm a selected simulated profile's values."""
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("✅ Use these values", callback_data=f"expuse:{profile_id}")],
            [InlineKeyboardButton("Change profile", callback_data="exp:back")],
            [InlineKeyboardButton("Custom values", callback_data="exp:custom")],
            [InlineKeyboardButton("✖ Cancel", callback_data="action:cancel")],
        ]
    )
...
def resolution_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Keep original", callback_data="res:keep")],
            [InlineKeyboardButton("iPhone-style 12 MP", callback_data="res:12mp")],
            [InlineKeyboardButton("Custom", callback_data="res:custom")],
        ]
    )


def resolution_fit_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Crop to fill", callback_data="resfit:crop")],
            [InlineKeyboardButton("Fit with padding", callback_data="resfit:fit")],
            [InlineKeyboardButton("« Back", callback_data="resfit:back")],
        ]
    )
