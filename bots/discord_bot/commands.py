...
from bots.shared.plan_config import exposure_resolution_kwargs
from bots.shared.status_events import emit_status
from bots.shared.temporary_files import TemporaryWorkspace
from bots.shared.zip_delivery import DIRECT_ATTACHMENT_WARNING, build_metadata_safe_zip
...
async def _do_export(interaction: discord.Interaction, session: BotSession) -> None:
    ...
    if session.config.get("direct_attachment"):
        ...
        await interaction.followup.send(
            content=f"{DIRECT_ATTACHMENT_WARNING}\n\nDone. {CAPTURE_DISCLAIMER}",
            files=files,
            ephemeral=True,
        )
    else:
        ws: TemporaryWorkspace = session.config["workspace"]
        diff = engine.build_diff(_preview_plan(session))
        zip_path = ws.output_dir / f"{Path(result.destination_path).stem}_lenstrace.zip"
        build_metadata_safe_zip(result, diff, zip_path)
        await interaction.followup.send(
            content=(
                f"Done — metadata-safe ZIP. {CAPTURE_DISCLAIMER}\n"
                "Download the ZIP as a file, extract it, then import the image into Photos."
            ),
            files=[discord.File(zip_path)],
            ephemeral=True,
        )
    _cleanup(session)
...
def _preview_plan(session: BotSession):
    ...
    return engine.build_change_plan(
        ...
        **exposure_resolution_kwargs(cfg),
    )
