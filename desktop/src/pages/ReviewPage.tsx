...
import type { CameraPreview, ChangeDiff } from '@/types';
...
export function ReviewPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        ...
        setCameraPreview(r.camera_preview ?? null);
      })
  ...
  return (
    <div className="content-inner">
      ...
      {cameraPreview && cameraPreview.lines.length > 0 && (
        <div className="panel">
          <h4 style={{ marginTop: 0 }}>Camera summary</h4>
          <pre className="summary-block">{cameraPreview.lines.join('\n')}</pre>
          {cameraPreview.exposure_source_label && (
            <p className="hint">{cameraPreview.exposure_source_label}</p>
          )}
        </div>
      )}
