...
from core.metadata_models import ChangePlan, DateStrategy, ExportResult, GPSData
...
def build_plan(request: ChangeRequest) -> ChangePlan:
    ...
    exposure_mode, exposure_custom = _resolve_exposure_request(request)
    resolution_mode, resolution_fit = _resolve_resolution_request(request)
    ...
    plan = _engine.build_change_plan(
        ...
        set_exposure=request.set_exposure,
        photographic_sensitivity=request.photographic_sensitivity,
        exposure_time=request.exposure_time,
        exposure_bias=request.exposure_bias,
        exposure_mode=exposure_mode,
        exposure_profile_id=request.exposure_profile_id,
        exposure_custom=exposure_custom,
        auto_fill_exposure=request.auto_fill_exposure,
...
def _resolve_exposure_request(request: ChangeRequest):
    """Map the request's exposure fields to an engine mode + custom values."""
    from core.exposure.models import FLASH_DID_NOT_FIRE, FLASH_FIRED, ExposureMode
    from core.exposure.validation import parse_shutter_speed

    if request.exposure_mode is None:
        return None, None
    mode = ExposureMode(request.exposure_mode)
    custom: dict | None = None
    if mode == ExposureMode.CUSTOM:
        custom = {}
        if request.exposure_iso is not None:
            custom["iso"] = request.exposure_iso
        if request.exposure_shutter:
            custom["exposure_time_seconds"] = parse_shutter_speed(request.exposure_shutter)
        if request.exposure_ev is not None:
            custom["exposure_bias"] = request.exposure_ev
        if request.exposure_flash_fired is not None:
            custom["flash"] = FLASH_FIRED if request.exposure_flash_fired else FLASH_DID_NOT_FIRE
    return mode, custom


def _resolve_resolution_request(request: ChangeRequest):
    from core.imaging.resolution_models import FitMode, ResolutionMode

    return ResolutionMode(request.resolution_mode), FitMode(request.resolution_fit)


def preview(request: ChangeRequest):
    """Return the before/after diff, destination filename, and camera preview."""
    from core.preview import build_camera_preview

    plan = build_plan(request)
    diff = _engine.build_diff(plan)
    camera_preview = build_camera_preview(plan)
    return diff, Path(plan.destination_path).name, camera_preview
