...
def preview_changes(request: ChangeRequest) -> PreviewResponse:
    try:
        diff, dest_name, camera_preview = preview_service.preview(request)
    except LensTraceError as exc:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=exc.user_message) from exc
    return PreviewResponse(
        diff=diff,
        destination_name=dest_name,
        disclaimer=preview_service.disclaimer(),
        camera_preview=CameraPreviewDTO(**camera_preview.model_dump()),
    )
